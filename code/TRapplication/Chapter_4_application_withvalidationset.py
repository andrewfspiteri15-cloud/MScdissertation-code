...
# taking a stratified sample of the training and test sets. !!! Changed this from 0.2 to 0.4 so that I can later then just split it again for the validation set
sss_train_testvalidation = StratifiedShuffleSplit(n_splits = 1, test_size = 0.4, random_state = 5) 
sss_test_validation = StratifiedShuffleSplit(n_splits = 1, test_size = 0.5, random_state = 5) 

# get train and testvalidation indices
for i, (train_index, testvalidation_index) in enumerate(sss_train_testvalidation.split(data_tensors, true_labels)):
     print(f"Fold {i}:")
     print(f"  Train: index={train_index}")
     print(f"  Test:  index={testvalidation_index}")

# convert from array type to list type
train_index = train_index.tolist()
testvalidation_index = testvalidation_index.tolist()
...
data_tensors_testandvalidation = []
true_labels_testandvalidation = []

for i in range(len(testvalidation_index)):
        # get the inputs; custom data object is a list of [inputs, labels] that need to be called to get them
        inputs = data_tensors[testvalidation_index[i]]
        labels = true_labels[testvalidation_index[i]]
        
        # save the inputs; get the tensor and label for each entry and save them as lists
        data_tensors_testandvalidation.append(inputs) # list of tensors
        true_labels_testandvalidation.append(labels)


# get test and validation indices
for i, (test_index, validation_index) in enumerate(sss_test_validation.split(data_tensors_testandvalidation, true_labels_testandvalidation)):
     print(f"Fold {i}:")
...
validation_index = validation_index.tolist()
...
for i in range(len(test_index)):
        # get the inputs; custom data object is a list of [inputs, labels] that need to be called to get them
        inputs = data_tensors_testandvalidation[test_index[i]]
        labels = true_labels_testandvalidation[test_index[i]]
        
        # save the inputs; get the tensor and label for each entry and save them as lists
        data_tensors_test.append(inputs) # list of tensors
        true_labels_test.append(labels)
        
data_tensors_validation = []
true_labels_validation = []

for i in range(len(validation_index)):
        # get the inputs; custom data object is a list of [inputs, labels] that need to be called to get them
        inputs = data_tensors_testandvalidation[validation_index[i]]
        labels = true_labels_testandvalidation[validation_index[i]]
        
        # save the inputs; get the tensor and label for each entry and save them as lists
        data_tensors_validation.append(inputs) # list of tensors
        true_labels_validation.append(labels)
...
array_img_labels_validation = np.array(true_labels_validation)
...
array_img_labels_validation = array_img_labels_validation.reshape(len(array_img_labels_test),1)
...
img_labels_validation_onehotencoded = OneHotEncoder.fit_transform(array_img_labels_validation)
...
# TRIED TO USE IT FOR FINDING THE LOG-LIKELIHOOD PERMINI-BATCH BUT ENDED UP BEING SLOWER THAN THE MANUAL APPROACH
...
validation_overall_log_likelihood_GAwithmomentum_total_list = []
...
for t1 in range(maxiter):
    ...
    with cf.ProcessPoolExecutor() as executor:
        validation_overall_log_likelihood_list = list(executor.map(cpdecomp_loglikelihood_MultiProcess_py.cpdecomp_loglikelihood_MultiProcess_logl, data_tensors_validation, img_labels_validation_onehotencoded, itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(Initial_blocks[0]), itertools.repeat(Initial_blocks[1]), itertools.repeat(Initial_blocks[2]), itertools.repeat(Initial_blocks[3]), itertools.repeat(Initial_blocks[4]), itertools.repeat(Initial_blocks[5]), itertools.repeat(Initial_blocks[6]), itertools.repeat(Initial_blocks[7]), itertools.repeat(Initial_blocks[8]), itertools.repeat(Initial_blocks[9]), itertools.repeat(Initial_blocks[10]), itertools.repeat(Initial_blocks[11]), itertools.repeat(Initial_blocks[12]), itertools.repeat(Initial_blocks[13]), itertools.repeat(Initial_blocks[14]), itertools.repeat(Initial_blocks[15]), itertools.repeat(Initial_blocks[16]), itertools.repeat(Initial_blocks[17]), itertools.repeat(lasso_regparameter))) # no regularization in the test
    
    validation_overall_log_likelihood_total = sum(validation_overall_log_likelihood_list)
    
    validation_overall_log_likelihood_GAwithmomentum_total_list.append(validation_overall_log_likelihood_total)
    
    print(f"Epoch {t1 + 1}, Validation log-likelihood {validation_overall_log_likelihood_total}")
    ...
    # Using the estimated Initial_blocks to create the weight tensors and predict labels for the image test and validation data
    ...
    validation_prob_allclasses_perdata = np.zeros(shape = (len(data_tensors_validation), 7)) # hosts the estimated probabilities for each observation
    ...
    with cf.ProcessPoolExecutor() as executor:
        twovariables_allvalidation_list = list(executor.map(predicted_labels_MultiProcess_py.test_predictedlabels_MultiProcess, data_tensors_validation, itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(Initial_blocks[0]), itertools.repeat(Initial_blocks[1]), itertools.repeat(Initial_blocks[2]), itertools.repeat(Initial_blocks[3]), itertools.repeat(Initial_blocks[4]), itertools.repeat(Initial_blocks[5]), itertools.repeat(Initial_blocks[6]), itertools.repeat(Initial_blocks[7]), itertools.repeat(Initial_blocks[8]), itertools.repeat(Initial_blocks[9]), itertools.repeat(Initial_blocks[10]), itertools.repeat(Initial_blocks[11]), itertools.repeat(Initial_blocks[12]), itertools.repeat(Initial_blocks[13]), itertools.repeat(Initial_blocks[14]), itertools.repeat(Initial_blocks[15]), itertools.repeat(Initial_blocks[16]), itertools.repeat(Initial_blocks[17])))
    ...
    validation_set_predicted_labels = [item[0] for item in twovariables_allvalidation_list]
    validation_prob_allclasses_perdata = np.concatenate([item[1] for item in twovariables_allvalidation_list], axis = 0) # [item[1] for item in twovariables_alltrain_list] returns list of arrays which np.concatenate combines to one array

    
    ### Calculating performance metrics
    ## Accuracy of train, test and validation data
    correct_train = 0
    wrong_train = 0
    ...
    wrong_validation = 0
    for i in range(len(true_labels_validation)):
        if true_labels_validation[i] == validation_set_predicted_labels[i]:
            correct_validation += 1
        else:
            wrong_validation += 1

    Validation_accuracy = (correct_validation / len(true_labels_validation)) * 100
    
    ## Confusion matrix of train, test and validation data
    ...
    cf_matrix_validation = confusion_matrix(true_labels_validation, validation_set_predicted_labels)
    ...
    precision_validation = precision_score(true_labels_validation, validation_set_predicted_labels, average='macro', zero_division=0)
    recall_validation = recall_score(true_labels_validation, validation_set_predicted_labels, average='macro', zero_division=0)
    f_measure_validation = 2 * (precision_validation * recall_validation / (precision_validation + recall_validation))
    ...
    array_img_labels_validation = array_img_labels_validation.reshape(-1,)
    ...
    macro_roc_auc_ovo_validation = roc_auc_score(array_img_labels_validation, validation_prob_allclasses_perdata, multi_class = "ovo", average = "macro")
    ...
    print('-----------------------')
    print('Validation accuracy for epoch %d: %d %%' % (t1 + 1, Validation_accuracy))
    print('Validation Precision %.3f, Recall %.3f, F-measure %.3f' % (precision_validation, recall_validation, f_measure_validation))
    print('Validation AUC %.3f' % (macro_roc_auc_ovo_validation))
    print('-----------------------')
    ...
    print('Epoch: %d, Validation Loss: %.3f' % (t1 + 1, validation_overall_log_likelihood_total / len(data_tensors_validation)))
    print('---------------------------------')
