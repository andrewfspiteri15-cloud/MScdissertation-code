...
file1 = 'C:\\Users\\Andrew\\Downloads\\Masters Data\\PostureRecognition - The third folder\\data_old_labels_dancers_1and2and3.csv'
...
def train_model(config, checkpoint_dir=None):
    ...
    transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize([0.4685, 0.4762, 0.4478], [0.2007, 0.2012, 0.2038])
    ])
    ...
    # !!!!! REMEMBER THAT THIS IS SEEDED HERE !!!!! randomly shuffle trainset according to a seed
    trainset_shuffled = trainset.data.iloc[np.random.RandomState(seed=42).permutation(len(trainset.data))]
    trainset_shuffled.reset_index(drop=True)
    
    trainset.dataset = trainset_shuffled
    
    #testset = DanceDataset(file2, transform = transform)
    
    # divide the data into 80% training, 10% validation and the other 10% as test
    test_abs = int(len(trainset) * 0.8)
    testval_abs = int(len(trainset) * 0.1)
    
    train_subset, testval_subset = random_split(
        trainset, [test_abs, len(trainset) - test_abs])
    
    test_subset, val_subset = random_split(
        testval_subset, [testval_abs, len(testval_subset) - testval_abs])
    # ^train_subset is unused
...
def test_accuracy(best_trial):
    ...
    # !!!!! REMEMBER THAT THIS IS SEEDED HERE !!!!! randomomly shuffle trainset according to a seed
    trainset_shuffled = trainset.data.iloc[np.random.RandomState(seed=42).permutation(len(trainset.data))]
    trainset_shuffled.reset_index(drop=True)
    ...
    testloader = torch.utils.data.DataLoader(
        test_subset, batch_size=4, shuffle=False, num_workers=2)
...
def main(num_samples=20, max_num_epochs=50, gpus_per_trial=2):
    ...
    config = {
        ...
        #"beta1": tune.uniform(0.01, 0.999), # if using AdamW with not default
        #"beta2": tune.uniform(0.01, 0.999) # if using AdamW with not default
    }
