...
#optimizer = optim.AdamW(net.parameters(), lr = 0.000411, betas = (0.26215, 0.74412), weight_decay = 0.00000215)
optimizer = optim.AdamW(net.parameters(), lr = 0.000113, weight_decay = 0.013869)
#optimizer = optim.SGD(net.parameters(), lr = 0.000113, momentum = 0.35885, weight_decay = 0.013869) #original model had used momentum of 0.9 and 0.0005 WD
...
transforms = { 'Train': torchvision.transforms.Compose([
                    ...
                    torchvision.transforms.Normalize([0.4559, 0.4694, 0.4374], [0.1870, 0.1918, 0.1934])
               ]),
...
layout = ['Dancers 1 and 3 (Training) - Dancer 2 (Test)']

# Iterate over the DataLoader for training data
data_train = DanceDataset("data_old_labels_dancers_1and3.csv", transform = transforms['Train'])
data_test = DanceDataset("data_old_labels_dancer_2.csv", transform = transforms['Test'])
...
criterion = nn.CrossEntropyLoss()
#criterion = nn.CrossEntropyLoss(weight = class_weights) # for weighed use this instead: 
