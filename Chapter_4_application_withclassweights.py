...
# Using CP decomposition with SGD + momentum WITH ADDED CLASS WEIGHTS AGAINST CLASS IMBALANCE and optional L1 regularization
...
import math
import cpdecomp_loglikelihood_MultiProcess_weighted_py # custom-made .py file containing a definition. This was done since there was an issue with getting multiprocessing to work and importing them like this fixed it
import derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py # custom-made .py file containing a definition. This was done since there was an issue with getting multiprocessing to work and importing them like this fixed it
...
data_train_customdataobject = DanceDataset("C:/Users/Andrew/Downloads/Masters Data/PostureRecognition - The third folder/data_old_labels_dancers_2and3.csv")
data_test_customdataobject = DanceDataset("C:/Users/Andrew/Downloads/Masters Data/PostureRecognition - The third folder/data_old_labels_dancer_1.csv")
...
## ADDING WEIGHTS TO THE LOG-LIKELIHOOD
#weighted cross entropy
y1 = data_train_customdataobject.img_labels
...
# BLOCK LOG-LIKELIHOOD CALCULATION (weighted)
# taking inspiration from torch cross_entropy (note that in this application, i have applied the reduction = 'none' version and NOT the 'mean' version that torch default uses) and https://stackoverflow.com/questions/68727252/cross-entropy-loss-with-weight-manual-calculation and https://medium.com/@zergtant/use-weighted-loss-function-to-solve-imbalanced-data-classification-problems-749237f38b75
def log_likelihood_innerproducts_weighted(data_tensors_train_random_batches, minibatch, class_weights, outer_product_factor_matrices_class1_rank1, outer_product_factor_matrices_class1_rank2, outer_product_factor_matrices_class1_rank3, outer_product_factor_matrices_class2_rank1, outer_product_factor_matrices_class2_rank2, outer_product_factor_matrices_class2_rank3,
    ...
    for i in range(len(data_tensors_train_random_batches[minibatch])):
        ...
        log_likelihood = class_weights[0]*img_labels_train_onehotencoded_random_batches[minibatch][i][0]*(bias_cpdecomp_LIST[0] + inner_product_class1 - ln_of_innerproduct_exponentials) + class_weights[1]*img_labels_train_onehotencoded_random_batches[minibatch][i][1]*(bias_cpdecomp_LIST[1] + inner_product_class2 - ln_of_innerproduct_exponentials) + class_weights[2]*img_labels_train_onehotencoded_random_batches[minibatch][i][2]*(bias_cpdecomp_LIST[2] + inner_product_class3 - ln_of_innerproduct_exponentials) + class_weights[3]*img_labels_train_onehotencoded_random_batches[minibatch][i][3]*(bias_cpdecomp_LIST[3] + inner_product_class4 - ln_of_innerproduct_exponentials) + class_weights[4]*img_labels_train_onehotencoded_random_batches[minibatch][i][4]*(bias_cpdecomp_LIST[4] + inner_product_class5 - ln_of_innerproduct_exponentials) + class_weights[5]*img_labels_train_onehotencoded_random_batches[minibatch][i][5]*(bias_cpdecomp_LIST[5] + inner_product_class6 - ln_of_innerproduct_exponentials) + class_weights[6]*img_labels_train_onehotencoded_random_batches[minibatch][i][6]*(0 - ln_of_innerproduct_exponentials)
...
# BIAS OF EVERY CLASS
def derivative_cportuckerdecomp_wrt_bias_vector_weighted(data, labels_onehotencoded, bias_vector, class_weights, inner_product_class1_list, inner_product_class2_list, inner_product_class3_list, inner_product_class4_list, inner_product_class5_list, inner_product_class6_list, ln_of_innerproduct_exponentials_list):
    ...
    for i in range(len(labels_onehotencoded)):
        ...
        for k in range(6):
            if k == 0:
                ...
                derivative_log_likelihood_bias_vector_ithdata[k] = class_weights[0]*labels_onehotencoded[i][0]*(term_1) - term_2 
                ...
                # Calculate the partial derivative of log-likelihood wrt weight value
                derivative_log_likelihood_bias_vector_ithdata[k] = class_weights[1]*labels_onehotencoded[i][1]*(term_1) - term_2
                ...
                # Calculate the partial derivative of log-likelihood wrt weight value
                derivative_log_likelihood_bias_vector_ithdata[k] = class_weights[2]*labels_onehotencoded[i][2]*(term_1) - term_2
                ...
                # Calculate the partial derivative of log-likelihood wrt weight value
                derivative_log_likelihood_bias_vector_ithdata[k] = class_weights[3]*labels_onehotencoded[i][3]*(term_1) - term_2
                ...
                # Calculate the partial derivative of log-likelihood wrt weight value
                derivative_log_likelihood_bias_vector_ithdata[k] = class_weights[4]*labels_onehotencoded[i][4]*(term_1) - term_2
                ...
                # Calculate the partial derivative of log-likelihood wrt weight value
                derivative_log_likelihood_bias_vector_ithdata[k] = class_weights[5]*labels_onehotencoded[i][5]*(term_1) - term_2
...
for t3 in range(maxiter):
    ...
    for minibatch in range(math.ceil(len(data_tensors_train) / batch_size)):
        ...
        for i in range(len(data_tensors_train_random_batches[minibatch])):
            ...
            # log-likelihood (weighted) per sample i
            log_likelihood_weighted = class_weights[0]*img_labels_train_onehotencoded_random_batches[minibatch][i][0]*(bias_cpdecomp_LIST[0] + inner_product_class1 - ln_of_innerproduct_exponentials) + class_weights[1]*img_labels_train_onehotencoded_random_batches[minibatch][i][1]*(bias_cpdecomp_LIST[1] + inner_product_class2 - ln_of_innerproduct_exponentials) + class_weights[2]*img_labels_train_onehotencoded_random_batches[minibatch][i][2]*(bias_cpdecomp_LIST[2] + inner_product_class3 - ln_of_innerproduct_exponentials) + class_weights[3]*img_labels_train_onehotencoded_random_batches[minibatch][i][3]*(bias_cpdecomp_LIST[3] + inner_product_class4 - ln_of_innerproduct_exponentials) + class_weights[4]*img_labels_train_onehotencoded_random_batches[minibatch][i][4]*(bias_cpdecomp_LIST[4] + inner_product_class5 - ln_of_innerproduct_exponentials) + class_weights[5]*img_labels_train_onehotencoded_random_batches[minibatch][i][5]*(bias_cpdecomp_LIST[5] + inner_product_class6 - ln_of_innerproduct_exponentials) + class_weights[6]*img_labels_train_onehotencoded_random_batches[minibatch][i][6]*(0 - ln_of_innerproduct_exponentials)
        ...
        #else:
        #    grace_bias = 0
        derivative_log_likelihood_bias_vector = derivative_cportuckerdecomp_wrt_bias_vector_weighted(data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], bias_cpdecomp_LIST, class_weights, inner_product_class1_list_new, inner_product_class2_list_new, inner_product_class3_list_new, inner_product_class4_list_new, inner_product_class5_list_new, inner_product_class6_list_new, ln_of_innerproduct_exponentials_list_new)
...
for t1 in range(maxiter):
    ...
    # Using the current iteration of parameters, calculate the log-likelihood (weighted)
    with cf.ProcessPoolExecutor() as executor:
        train_overall_log_likelihood_list = list(executor.map(cpdecomp_loglikelihood_MultiProcess_weighted_py.cpdecomp_loglikelihood_MultiProcess_logl_weighted, data_tensors_train, img_labels_train_onehotencoded, itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[0]), itertools.repeat(Initial_blocks[1]), itertools.repeat(Initial_blocks[2]), itertools.repeat(Initial_blocks[3]), itertools.repeat(Initial_blocks[4]), itertools.repeat(Initial_blocks[5]), itertools.repeat(Initial_blocks[6]), itertools.repeat(Initial_blocks[7]), itertools.repeat(Initial_blocks[8]), itertools.repeat(Initial_blocks[9]), itertools.repeat(Initial_blocks[10]), itertools.repeat(Initial_blocks[11]), itertools.repeat(Initial_blocks[12]), itertools.repeat(Initial_blocks[13]), itertools.repeat(Initial_blocks[14]), itertools.repeat(Initial_blocks[15]), itertools.repeat(Initial_blocks[16]), itertools.repeat(Initial_blocks[17]), itertools.repeat(lasso_regparameter)))
    ...
    with cf.ProcessPoolExecutor() as executor:
        test_overall_log_likelihood_list = list(executor.map(cpdecomp_loglikelihood_MultiProcess_weighted_py.cpdecomp_loglikelihood_MultiProcess_logl_weighted, data_tensors_test, img_labels_test_onehotencoded, itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[0]), itertools.repeat(Initial_blocks[1]), itertools.repeat(Initial_blocks[2]), itertools.repeat(Initial_blocks[3]), itertools.repeat(Initial_blocks[4]), itertools.repeat(Initial_blocks[5]), itertools.repeat(Initial_blocks[6]), itertools.repeat(Initial_blocks[7]), itertools.repeat(Initial_blocks[8]), itertools.repeat(Initial_blocks[9]), itertools.repeat(Initial_blocks[10]), itertools.repeat(Initial_blocks[11]), itertools.repeat(Initial_blocks[12]), itertools.repeat(Initial_blocks[13]), itertools.repeat(Initial_blocks[14]), itertools.repeat(Initial_blocks[15]), itertools.repeat(Initial_blocks[16]), itertools.repeat(Initial_blocks[17]), itertools.repeat(lasso_regparameter)))
    ...
    for b in range(len(Initial_blocks)):
        ...
        for minibatch in range(math.ceil(len(data_tensors_train) / batch_size)):
            ## Calculate the log-likelihood and inner products etc
            block_log_likelihood_list_total, inner_product_class1_list, inner_product_class2_list, inner_product_class3_list, inner_product_class4_list, inner_product_class5_list, inner_product_class6_list, ln_of_innerproduct_exponentials_list = log_likelihood_innerproducts_weighted(data_tensors_train_random_batches, minibatch, class_weights, outer_product_factor_matrices_class1_rank1, outer_product_factor_matrices_class1_rank2, outer_product_factor_matrices_class1_rank3, outer_product_factor_matrices_class2_rank1, outer_product_factor_matrices_class2_rank2, outer_product_factor_matrices_class2_rank3,
                    outer_product_factor_matrices_class3_rank1, outer_product_factor_matrices_class3_rank2, outer_product_factor_matrices_class3_rank3, outer_product_factor_matrices_class4_rank1, outer_product_factor_matrices_class4_rank2, outer_product_factor_matrices_class4_rank3,
                    outer_product_factor_matrices_class5_rank1, outer_product_factor_matrices_class5_rank2, outer_product_factor_matrices_class5_rank3, outer_product_factor_matrices_class6_rank1, outer_product_factor_matrices_class6_rank2, outer_product_factor_matrices_class6_rank3)
            ...
            if b == 0:
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix1_CLASS1_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix1_MultiProcess_class1_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[0]), itertools.repeat(Initial_blocks[1]), itertools.repeat(Initial_blocks[2]), inner_product_class1_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix2_CLASS1_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix2_MultiProcess_class1_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[0]), itertools.repeat(Initial_blocks[1]), itertools.repeat(Initial_blocks[2]), inner_product_class1_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix3_CLASS1_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix3_MultiProcess_class1_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[0]), itertools.repeat(Initial_blocks[1]), itertools.repeat(Initial_blocks[2]), inner_product_class1_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix1_CLASS2_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix1_MultiProcess_class2_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[3]), itertools.repeat(Initial_blocks[4]), itertools.repeat(Initial_blocks[5]), inner_product_class2_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix2_CLASS2_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix2_MultiProcess_class2_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[3]), itertools.repeat(Initial_blocks[4]), itertools.repeat(Initial_blocks[5]), inner_product_class2_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix3_CLASS2_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix3_MultiProcess_class2_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[3]), itertools.repeat(Initial_blocks[4]), itertools.repeat(Initial_blocks[5]), inner_product_class2_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix1_CLASS3_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix1_MultiProcess_class3_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[6]), itertools.repeat(Initial_blocks[7]), itertools.repeat(Initial_blocks[8]), inner_product_class3_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix2_CLASS3_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix2_MultiProcess_class3_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[6]), itertools.repeat(Initial_blocks[7]), itertools.repeat(Initial_blocks[8]), inner_product_class3_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix3_CLASS3_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix3_MultiProcess_class3_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[6]), itertools.repeat(Initial_blocks[7]), itertools.repeat(Initial_blocks[8]), inner_product_class3_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix1_CLASS4_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix1_MultiProcess_class4_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[9]), itertools.repeat(Initial_blocks[10]), itertools.repeat(Initial_blocks[11]), inner_product_class4_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix2_CLASS4_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix2_MultiProcess_class4_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[9]), itertools.repeat(Initial_blocks[10]), itertools.repeat(Initial_blocks[11]), inner_product_class4_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix3_CLASS4_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix3_MultiProcess_class4_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[9]), itertools.repeat(Initial_blocks[10]), itertools.repeat(Initial_blocks[11]), inner_product_class4_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix1_CLASS5_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix1_MultiProcess_class5_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[12]), itertools.repeat(Initial_blocks[13]), itertools.repeat(Initial_blocks[14]), inner_product_class5_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix2_CLASS5_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix2_MultiProcess_class5_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[12]), itertools.repeat(Initial_blocks[13]), itertools.repeat(Initial_blocks[14]), inner_product_class5_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix3_CLASS5_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix3_MultiProcess_class5_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[12]), itertools.repeat(Initial_blocks[13]), itertools.repeat(Initial_blocks[14]), inner_product_class5_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix1_CLASS6_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix1_MultiProcess_class6_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[15]), itertools.repeat(Initial_blocks[16]), itertools.repeat(Initial_blocks[17]), inner_product_class6_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix2_CLASS6_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix2_MultiProcess_class6_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[15]), itertools.repeat(Initial_blocks[16]), itertools.repeat(Initial_blocks[17]), inner_product_class6_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
                ...
                with cf.ProcessPoolExecutor() as executor:
                    derivative_log_likelihood_weight_factormatrix3_CLASS6_list = list(executor.map(derivative_cpdecomp_wrt_weight_factor_matrices_MultiProcess_weighted_py.derivative_cpdecomp_wrt_weight_factor_matrix3_MultiProcess_class6_weighted, data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], itertools.repeat(cpdecomp_rank), itertools.repeat(bias_cpdecomp_LIST), itertools.repeat(class_weights), itertools.repeat(Initial_blocks[15]), itertools.repeat(Initial_blocks[16]), itertools.repeat(Initial_blocks[17]), inner_product_class6_list, ln_of_innerproduct_exponentials_list, itertools.repeat(lasso_regparameter)))
    ...
    for minibatch in range(math.ceil(len(data_tensors_train) / batch_size)):
        ...
        #else:
        #    grace_bias = 0
        derivative_log_likelihood_bias_vector = derivative_cportuckerdecomp_wrt_bias_vector_weighted(data_tensors_train_random_batches[minibatch], img_labels_train_onehotencoded_random_batches[minibatch], bias_cpdecomp_LIST, class_weights, inner_product_class1_list, inner_product_class2_list, inner_product_class3_list, inner_product_class4_list, inner_product_class5_list, inner_product_class6_list, ln_of_innerproduct_exponentials_list)
